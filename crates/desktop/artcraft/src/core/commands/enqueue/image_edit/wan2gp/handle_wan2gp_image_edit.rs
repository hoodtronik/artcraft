...
/// Handle image edit via the local Wan2GP bridge.
///
/// Image editing models in Wan2GP (Flux 2 Klein, Flux 2 Dev, Qwen edit) accept
/// a source image + prompt to produce a modified image. The bridge accepts
/// the image as base64 in the `image_start` field with `output_type: "image"`.
pub async fn handle_wan2gp_image_edit(
  ...
  // Process scene image (primary edit target)
  let mut image_start_b64: Option<String> = None;
  if let Some(ref scene_token) = request.scene_image_media_token {
    ...
    image_start_b64 = Some(b64_engine.encode(&bytes));
    info!("Encoded scene image for Wan2GP edit: {} bytes", bytes.len());
  }

  // Process additional image media tokens
  let mut image_refs_b64: Vec<String> = Vec::new();
  if let Some(ref tokens) = request.image_media_tokens {
    for (i, token) in tokens.iter().enumerate() {
      ...
      let bytes = tokio::fs::read(local_file.path()).await.map_err(GenerateError::IoError)?;
      if image_start_b64.is_none() && i == 0 {
        // First image becomes the primary edit target if no scene image
        image_start_b64 = Some(b64_engine.encode(&bytes));
      } else {
        image_refs_b64.push(b64_engine.encode(&bytes));
      }
      info!("Encoded image {} for Wan2GP edit: {} bytes", i, bytes.len());
    }
  }
  ...
  let gen_request = GenerateRequest {
    ...
    video_length: None, // Not applicable for image edits
    image_start: image_start_b64,
    image_end: None,
    image_refs: if image_refs_b64.is_empty() { None } else { Some(image_refs_b64) },
    profile_params: wan2gp_settings.profile_params(),
    extra_params: None,
  };

  info!("Submitting image edit to Wan2GP bridge: model={}", wan2gp_model);

  // Submit the task
  let gen_response = client.generate(&gen_request).await.map_err(|e| {
    error!("Wan2GP image edit submission failed: {}", e);
    GenerateError::AnyhowError(anyhow!("Wan2GP image edit failed: {}", e))
  })?;

  info!("Wan2GP image edit task created: task_id={}", gen_response.task_id);
